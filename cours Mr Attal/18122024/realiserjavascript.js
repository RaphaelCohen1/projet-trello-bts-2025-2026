async function generateJsCode_() {
    ...
    for (let i = 0; i < selects.length; i += 2) {
        ...
        if (entiteMere !== 'Sélectionner une entité' && entiteMere !== '' &&
         ...
         //   htmlCode += await fetchHtmlDropdown(entiteMere) + '\n\n';
            ...
            htmlCode += await listerLesProprietes_(entiteMere,entiteFille) + '\n\n';
        }
    }
...
async function listerLesProprietes_(entiteMere, entiteFille) {
    try {
        // Construire l'URL avec les deux paramètres d'entités
        const url = `realiserJavascript.php?action=genererJavascript&entiteMere=${encodeURIComponent(entiteMere)}&entiteFille=${encodeURIComponent(entiteFille)}`;
