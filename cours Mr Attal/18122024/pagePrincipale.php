...
header('Content-Type: application/json'); // Déclare que la réponse est au format JSON
try {
    $pdo = new PDO('mysql:host=localhost;dbname=MITSUYAN', 'votre_utilisateur', 'votre_mot_de_passe');
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

    if ($_GET['action'] === 'chargerEntites') {
        $stmt = $pdo->query('SELECT * FROM t_ENTITE');
        $result = $stmt->fetchAll(PDO::FETCH_ASSOC);

        echo json_encode($result); // Renvoie les données au format JSON
    }
} catch (PDOException $e) {
    echo json_encode(['error' => $e->getMessage()]); // En cas d'erreur, renvoie un message JSON
}
